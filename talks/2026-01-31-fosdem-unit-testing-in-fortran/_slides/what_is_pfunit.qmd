...
- Follows the style JUnit. i.e Directives
  ...
  JUnit) label code sections for the pre-processor.
- Uses a pre-processor to convert `.pf` files to `.f90`.
- F2003 object oriented features and a smattering of F2008 features, thus
  requires relatively recent versions of compilers.

## Syntax {footer=false}

`.pf` syntax is similar to `.f90` with pre-processor directives, such as `@Test`
and `@assertEqual`.
...
## Assert pre-processor directives

```{=html}
<iframe
    height="70%"
    src="https://pfunit.sourceforge.net/page_Assert.html"
    title="pFUnit assert pre-processor directives"
    width="100%"
>
</iframe>
```
